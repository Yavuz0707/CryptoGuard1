...
namespace CryptoGuard.MAUI.ViewModels
{
    ...
    public partial class ProfileViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private ObservableCollection<Coin> topCoins = new();
        ...
        public ProfileViewModel(IUserService userService, IFeedPostService feedPostService, IPortfolioService portfolioService, ICoinLoreService coinLoreService)
        {
            ...
            Microsoft.Maui.Controls.MessagingCenter.Subscribe<object>(this, "FavoritesChanged", async (sender) =>
            {
                if (UserContext.CurrentUserId != null)
                    await LoadUserCoinsAsync(UserContext.CurrentUserId.Value);
            });
        }
        ...
        private async void LoadUserProfile(string username)
        {
            ...
            if (!string.IsNullOrEmpty(username))
            {
                ...
                if (user != null)
                {
                    ...
                    await LoadUserCoinsAsync(user.Id);
                }
            }
        }

        public async Task LoadUserCoinsAsync(int userId)
        {
            // Favori coinler
            var favCoinIds = await _portfolioService.GetFavoriteCoinsAsync(userId);
            var allCoins = (await _coinLoreService.GetTopCoinsAsync(100)).ToList();
            FavoriteCoins = new ObservableCollection<Coin>(allCoins.Where(c => favCoinIds.Contains(c.Id)));
            TopCoins = new ObservableCollection<Coin>(allCoins.Take(10));
        }
        ...
        {
            if (UserContext.CurrentUserId == null || string.IsNullOrEmpty(Username)) return;
            var user = await _userService.GetUserByUsernameAsync(Username);
            if (user == null) return;
            await _userService.FollowUserAsync(UserContext.CurrentUserId.Value, user.Id);
            IsFollowing = true;
            FollowersCount = await _userService.GetFollowersCountAsync(user.Id);
        }
        ...
        private async Task UnfollowUserAsync()
        {
            ...
            var user = await _userService.GetUserByUsernameAsync(Username);
            if (user == null) return;
            await _userService.UnfollowUserAsync(UserContext.CurrentUserId.Value, user.Id);
            IsFollowing = false;
            FollowersCount = await _userService.GetFollowersCountAsync(user.Id);
        }
