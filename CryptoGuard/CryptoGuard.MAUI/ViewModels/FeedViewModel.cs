...
namespace CryptoGuard.MAUI.ViewModels
{
    public partial class FeedViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private bool showOnlyFollowing = false;
        ...
        public ObservableCollection<FeedPost> FilteredFeedPosts =>
            ShowOnlyFollowing
                ? new ObservableCollection<FeedPost>(FeedPosts.Where(p => FollowingUserIds.Contains(p.UserId)))
                : FeedPosts;

        public List<int> FollowingUserIds { get; set; } = new();
        ...
        public async Task LoadFeedAsync()
        {
            ...
            OnPropertyChanged(nameof(FilteredFeedPosts));
        }
        ...
        public async Task LoadFollowingUserIdsAsync()
        {
            if (UserContext.CurrentUserId == null) return;
            var following = await _userService.GetFollowingAsync(UserContext.CurrentUserId.Value);
            FollowingUserIds = following.Select(u => u.Id).ToList();
            OnPropertyChanged(nameof(FilteredFeedPosts));
        }

        partial void OnShowOnlyFollowingChanged(bool value)
        {
            OnPropertyChanged(nameof(FilteredFeedPosts));
        }
    }
